...
def parse_wait_time(response: str) -> int:
    """
    Парсит ответ FunPay на запрос о поднятии лотов и возвращает время ожидания в секундах.

    :param response: текст ответа FunPay.
    :type response: :obj:`str`

    :return: примерное время ожидания до следующего поднятия лотов (в секундах).
    :rtype: :obj:`int`
    """
...
def parse_currency(s: str) -> Currency:
    """
    Определяет объект валюты по строковому обозначению.

    :param s: строковое обозначение валюты.
    :type s: :obj:`str`

    :return: объект валюты.
    :rtype: :class:`FunPayAPI.common.enums.Currency`
    """
...
def parse_funpay_datetime(date_text: str) -> datetime:
    """
    Парсит строку даты/времени FunPay в объект datetime (московское время, UTC+3).

    Поддерживаемые форматы:
    - «сегодня, ЧЧ:ММ» / «today, ЧЧ:ММ»
    - «вчера, ЧЧ:ММ» / «yesterday, ЧЧ:ММ»
    - «ДД месяца, ЧЧ:ММ»
    - «ДД месяца ГГГГ, ЧЧ:ММ»

    :param date_text: строка с датой/временем.
    :type date_text: :obj:`str`
...
class RegularExpressions(object):
    """
    В данном классе хранятся скомпилированные регулярные выражения, описывающие системные сообщения FunPay.
